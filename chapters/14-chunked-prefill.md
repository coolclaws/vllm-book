...
下图对比了无 Chunked Prefill 与有 Chunked Prefill 时的执行时序：

```mermaid
flowchart TD
    subgraph NCP["无 Chunked Prefill"]
        N1["迭代 1: 50 decode tokens"] --> N2["迭代 2: 32000 prefill tokens\n+ 50 decode tokens"]
        N2 --> N3["⚠️ 数百毫秒延迟\ndecode 用户卡顿"]
        N3 --> N4["迭代 3: 51 decode tokens"]
    end
    subgraph CP["有 Chunked Prefill"]
        C1["迭代 1: 50 decode\n+ 1998 prefill chunk1"] --> C2["迭代 2: 50 decode\n+ 1998 prefill chunk2"]
        C2 --> C3["... 每轮 ≤ 2048 tokens\ndecode 用户无感知"]
        C3 --> C4["迭代 16: prefill 完成\n开始正常 decode"]
    end
```
...
会的。原本一次完成的 prefill 现在被拆分为多轮迭代，TTFT 会增加。但这是有意的权衡——牺牲新请求的 TTFT 来保护已有 decode 用户的 inter-token 延迟平滑性。在交互式服务中，用户对"打字流畅度"的感知远比"首次响应快几百毫秒"更敏感。

---

**Q：Chunked Prefill 和普通的限制 max_num_batched_tokens 有什么区别？**

限制 `max_num_batched_tokens` 不做分块时，如果一个 prompt 超过预算，要么拒绝、要么在一轮中超预算处理。Chunked Prefill 的关键在于允许一个请求的 prefill 跨越多轮迭代完成，通过 `num_computed_tokens` 追踪进度，真正实现了"分期付款"式的计算。

---

**Q：Chunk 大小是固定的吗？**

不是。每轮的实际 chunk 大小取决于剩余 token 预算（总预算减去 decode 序列占用的部分）。如果某轮有较多 decode 序列，留给 prefill 的预算就小，chunk 也就更小。最后一个 chunk 通常小于标准大小。

---

**Q：多个请求同时需要 prefill 时如何分配预算？**

调度器通常按优先级逐个分配。默认策略倾向于先完成已经开始 prefill 的请求（减少其 TTFT），剩余预算再分给下一个等待的请求。这避免了多个请求同时"半完成"的状态。
