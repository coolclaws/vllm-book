...
下面的时序图展示了 Continuous Batching 中 batch 的动态变化——序列随时进出：

```mermaid
flowchart LR
    subgraph T1["迭代 1"]
        R1A["请求A"] ~~~ R1B["请求B"] ~~~ R1C["请求C"]
    end
    subgraph T2["迭代 2\nA完成，D加入"]
        R2B["请求B"] ~~~ R2C["请求C"] ~~~ R2D["请求D"]
    end
    subgraph T3["迭代 3\nB完成，E加入"]
        R3C["请求C"] ~~~ R3D["请求D"] ~~~ R3E["请求E"]
    end
    T1 --> T2 --> T3
```
...
下图对比了静态批处理和 Continuous Batching 的 GPU 利用率变化：
...
不完全一样。Dynamic Batching 通常指在 batch 开始前根据当前队列动态决定 batch 大小，但一旦开始处理就不会中途改变。Continuous Batching（也叫 iteration-level scheduling）更进一步——在每一次迭代边界都可以增减序列，粒度更细、利用率更高。

---

**Q：Prefill 和 Decode 混在同一个 batch 中不会互相干扰吗？**

计算上不会。vLLM 的 attention kernel 通过 metadata 区分每个序列的状态，能在同一次调用中正确处理不同长度的 query。性能上反而有好处：prefill 是计算密集型，decode 是内存带宽密集型，两者混合可以更充分地利用 GPU 的计算和带宽资源。

---

**Q：如果所有请求都是长文本生成，Continuous Batching 还有优势吗？**

优势会减弱但仍然存在。即使所有请求生成长度相近，请求到达时间不同也会导致完成时间错开。Continuous Batching 能立即回收完成请求的资源并接纳新请求，而静态批处理必须等最后一个请求完成。只有在所有请求完全同步开始和结束时，两者才没有差异。

---

**Q：max_num_batched_tokens 设得越大，吞吐量就越高吗？**

不一定。增大该参数允许更多 token 并行处理，但也会增加单次迭代的延迟，导致所有 decode 请求的 inter-token 延迟增大。此外，过大的 batch 可能导致 GPU 显存不足触发抢占。最优值取决于模型大小、GPU 型号和延迟要求。
